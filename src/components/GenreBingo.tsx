...
export function GenreBingo({
  ...
  const selectedCellData =
    selectedCell !== null ? board.cells[selectedCell] : null;
          ...
          <div className="flex items-center gap-2">
            {completedLines.length > 0 && (
              <Badge variant="coral" className="flex items-center gap-1">
                ...
                {completedLines.length === 1 ? "Line" : "Lines"}
              </Badge>
            )}
            {board.isFullBoard && (
              <motion.div
        ...
        <div className="mt-4">
          <div className="flex justify-between text-sm mb-1">
            <span className="text-stone-600">
              {completedCount} / {totalCells} genres
            </span>
          ...
          {board.cells.map((cell, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Genre icon */}
                <span
                  className={`text-2xl ${board.size === 5 ? "text-xl" : ""}`}
                >
                  {cell.genre.icon}
                </span>
              ...
              <div className="space-y-4">
                <p className="text-stone-600">
                  ...
                  <span className="font-semibold">
                    {selectedCellData.genre.name}
                  </span>{" "}
                  book to complete this cell!
                </p>
                <Button
                  variant="primary"
                  onClick={() => {
...
export function MiniBingo({ board, onClick, className = "" }: MiniBingoProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`grid gap-0.5 ${board.size === 3 ? "grid-cols-3" : "grid-cols-5"}`}
      >
        {board.cells.map((cell, index) => (
          <div
            key={index}
...
export function createBingoBoard(size: 3 | 5 = 3): BingoBoard {
  ...
  const cells: BingoCell[] = shuffledGenres
    .slice(0, size * size)
    .map((genre) => ({
      genre,
      completed: false,
...
export const SAMPLE_BINGO_BOARD: BingoBoard = {
  ...
  cells: [
    {
      genre: BINGO_GENRES[0],
      completed: true,
      bookTitle: "Harry Potter",
      completedDate: "2024-11-15",
    },
    {
      genre: BINGO_GENRES[1],
      completed: true,
      bookTitle: "Mysteries of the Library",
      completedDate: "2024-11-20",
    },
    {
      genre: BINGO_GENRES[2],
      completed: true,
      bookTitle: "Island Adventures",
      completedDate: "2024-11-25",
    },
    { genre: BINGO_GENRES[3], completed: false },
    {
      genre: BINGO_GENRES[4],
      completed: true,
      bookTitle: "The Best Day Ever",
      completedDate: "2024-11-28",
    },
    { genre: BINGO_GENRES[5], completed: false },
    {
      genre: BINGO_GENRES[6],
      completed: true,
      bookTitle: "Diary of a Silly Kid",
