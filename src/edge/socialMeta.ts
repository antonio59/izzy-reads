// Shared social-crawler meta handler.
// Used by both the Cloudflare Pages middleware (functions/_middleware.ts)
// and the Workers static-assets entrypoint (workers/index.ts).
// Returns a meta-HTML Response for link-preview bots on meta-able paths,
// or null when the request should fall through to normal asset serving.
...
/**
 * Return meta-HTML for social crawlers on supported paths, or null to let the
 * request continue to normal asset/SPA serving.
 */
export function socialMetaResponse(request: Request): Response | null {
